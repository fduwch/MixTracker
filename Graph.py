...
# List of known Tornado Cash addresses
...
def build_address_graph(address, start_block=None, end_block=None, max_txs=None):
    """Builds a graph for an address, optionally filtered by block numbers and tx count."""
    ...
    # Cap by the number of transactions, keeping the most recent ones
...
def build_address_graph_by_tx_volume(address, reference_timestamp, mode, ratio, max_txs=None):
    """
    Builds a graph by expanding the transaction history based on a ratio of total transaction volume.
    - mode='deposit': Expands forward in time.
    - mode='withdrawal': Expands backward in time.
    """
    all_txs_df = _load_all_txs_for_address(address)
    if all_txs_df is None:
        return None, None
    
    # A stable sort that is compatible with many pandas versions.
    # We sort by timestamp, then by the original order (index) to break ties.
...
def extract_edge_features(df, addr_to_index):
    ...
    # Replace any NaNs that may have occurred
...
def extract_node_features(df):
    """
    Vectorized feature extraction for nodes.
    This is significantly faster than iterating through each address.
    """
    # 预处理: 单位换算
    ...
    # --- New: Mixer Interaction Features ---
    ...
    # 使用图中的所有节点来重构索引，确保所有节点都有特征
    all_nodes = pd.unique(df[['from', 'to']].values.ravel('K'))
    feature_df = all_features.reindex(all_nodes).fillna(0)
    
    # 确保返回的DataFrame有'address'列
